...
float targetDistance = 9.0, distanceRampDn = 2.5, distanceBreak = 0.0, distanceCameraOn = 0.5; // distanceBreak = how many m before finish line it starts to break
//DistanceCameraOn must be smaller than distanceRampDn
//distanceRampDn must be smaller than Beginning Segment (4.07) for 9m
...
float targetVelocityMid = 1.0; // in m/s
...
const int maxServoAngCamera = 99;
const int minServoAngCamera = 79;
const int midServoAngCamera = 89;
...
void setup()
{
  ...
  */
  ...
  analogReference(INTERNAL1V1); // for audio communication w/ smartphone
...
void loop()
{
  if (doneY_0 == false) {
    ...
    if (ready == false) {
      ...
      //After Beginning Segment (MIDDLE SEGMENT)
      ...
      //(Target - RampDnDistance) to (Target - CameraOn)
      while (distance < (2*beginningSegment + midSegment - distanceBreak - distanceCameraOn) && distance >= (2*beginningSegment + midSegment - distanceBreak - distanceRampDn)) {
        runMotorInput();
        setESCSpeed((int)(ESCSpeedCmd));
      ...
      // ADCCode = analogRead(audioPin);
      // count =0;      
      
      //(Target - CameraOn) to (Target)
      while (distance < (2*beginningSegment + midSegment - distanceBreak) && distance >= (2*beginningSegment + midSegment - distanceBreak - distanceCameraOn)) {
        runMotorInput();
        setESCSpeed((int)(ESCSpeedCmd));
        //Steering by Camera
        cameraSteering();
...
void cameraSteering() {
  float audioLSBmv = 0.115; // each 16 code change in adndroid app produces 115mV output at audio port
  float audioLSB = 0.115/1.1*1024; // ADC uses 1.1V reference
  int ADCCode, count, maxAvgCount=10;
  long int audioCode;  // need long int when adding large ADC output codes for count of 40 or more
  audioCode = analogRead(audioPin);
  while(ADCCode > 0.5*audioLSB & count < maxAvgCount) {
    ADCCode = analogRead(audioPin);
    audioCode = audioCode+ADCCode;
    count = count+1;
    // Serial.println(count);
    // Serial.print(audioCode);
    // Serial.print("=====\n");
  }

  if (count == maxAvgCount){
   audioCode = audioCode/maxAvgCount;
  }
  else {
    audioCode = 0;
  }
 ...
 // android max output is 0.776V, AREF = 1.1V

 

 // Serial.print("audioCode is: ");
 // Serial.print(audioCode);
 
 // audioCodeReMap = map(audioCode, 0, 1023, 0, fullscale);

 // convert to 7 bit code:

 // audioCode7 = audioCodeReMap/8;


 if (audioCode <(int)0.5*audioLSB){
  // Serial.print("Obj not detected\n");
  // delay(10);
 }

 if (audioCode>=0.5*audioLSB & audioCode < 1.5*audioLSB){
  // osc: hi = 115mV
  // Serial.print("Sharp Left Turn:");
  steeringServo.write(maxServoAngCamera);
  // Serial.print(audioCode);
  // Serial.print("\t");
  // Serial.print(count);
  // Serial.print("=======================\n");
 ...
 if (audioCode>=1.5*audioLSB & audioCode < 2.5*audioLSB){
  // scope: hi = 232mV
  // Serial.print("Left Turn\n");Serial.print("\t");
  steeringServo.write((int)((maxServoAngCamera-midServoAngCamera)/2));
  // delay(10);
 }

 if (audioCode>=2.5*audioLSB & audioCode < 3.5*audioLSB){
  // scope: hi = 344mV
  // Serial.print("Center (go Straight):");
  steeringServo.write(midServoAngCamera);
  // Serial.print(audioCode);Serial.print("\t");
  // Serial.print(audioCodeReMap);Serial.print("\t");
  // Serial.print(audioCode7);
  // Serial.print("\n");
  // delay(10);
 }

 if (audioCode>=3.5*audioLSB & audioCode < 5*audioLSB){
  // scope: hi = 464mV
  // Serial.print("Right Turn:"); 
  steeringServo.write((int)((midServoAngCamera - minServoAngCamera)/2));
  // Serial.print(audioCode);Serial.print("\t");
  // Serial.print(audioCodeReMap);Serial.print("\t");
  // Serial.print(audioCode7);
  //Serial.print("\n");
 ...
 if (audioCode >=5*audioLSB & audioCode < 6.4*audioLSB){

  // scope: hi = 656mV.  audioCode = 430; expect 5*115 = 575, skip code: 6*115=690mV
  //Serial.print("Sharp Right Turn: ");
  steeringServo.write(minServoAngCamera);
  //Serial.print(audioCode); Serial.print("\t");
  ...
  if (audioCode >=6.4*audioLSB){
  // scope: hi =776mV ; expect 7*115 = 805mV
  //Serial.print("Target Reached !! \n");
  setESCSpeed(0);
    distance = 2*beginningSegment + midSegment - distanceBreak; // If camera says target is reached, distance will be set to exit the while loop and stop the car.
  //Serial.print(audioCode);
  //Serial.print("\n");
  //delay(1000);
  } //
