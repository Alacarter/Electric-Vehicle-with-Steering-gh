...
float targetDistance = 9.0, distanceRampDn = 3.0, distanceBreak = 0.0; // distanceBreak = how many m before finish line it starts to break
...
const float k_p = 0.5; // 0.5 seems to work best. Maybe lower would be good as well.
const float k_i = 0.2;
//===||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||===
...
int integralTau = 3; //number of readings to summation
...
void runMotorInput() { //makes sure motor stops at target distance
  distance = 0.5*distanceR + 0.5*distanceL;
   ...
   if (distance > targetDistance-distanceRampDn){
     setESCSpeed(8); //7 works when car is in the air. Slowest possible is 8.
   }
   /*
