...
float targetDistance = 9, distanceRampDn = 0.0, distanceBreak = 0.0;
...
if (ready == false) {
 ...
 while (distance < (2*beginningSegment + midSegment) && distance >= (beginningSegment + midSegment))
 {
   runMotorInput();
   // Direction Control
   doGyro();
...
void calcTurnAng() {
  turnAng = atan(((targetDistance-midSegment)/2)/horizOffset);
  turnAng = (turnAng * 180)/3.1416; // convert to degrees
  if (turnAng < 0)
  {
